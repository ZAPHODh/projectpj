...
export function WorkingHoursInput({ initialWorkingHours, onChange }: WorkingHoursInputProps) {
    const [localWorkingHours, setLocalWorkingHours] = useState<TWorkingHours>({ ...initialWorkingHours })
    ...
    const handleApply = () => {
        ...
        for (const dayId in updatedWorkingHours) {
            ...
            const isDayActive =
                localWorkingHours[Number.parseInt(dayId)].from > 0 || localWorkingHours[Number.parseInt(dayId)].to > 0
